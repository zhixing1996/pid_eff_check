...
EvtPreSlt* EvtPreSlt::m_pointer = 0;
EvtPreSlt* EvtPreSlt::instance() {
    if(!m_pointer) m_pointer = new EvtPreSlt();
    return m_pointer;
}

void EvtPreSlt::PiPiPiPiSel(HepLorentzVector p4Lab, Vint iGood) {
    PiPiPiPiSelValid = false;
    if (iGood.size() != 4) return ;
    ...
    if (n_pip != 2 || n_pim != 2) return ;
    chi2_vf = vertex_fit(vwtrkpara_pi, birth);
    if (chi2_vf == -999.) return ;
    chi2_kf = kinematic_fit(vwtrkpara_pi, p4Lab);
    if (chi2_kf == -999.) return ;
    PiPiPiPiSelValid = true;
    return ;
}
